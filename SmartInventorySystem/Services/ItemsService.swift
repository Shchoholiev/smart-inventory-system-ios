...
class ItemsService: ServiceBase {
    ...
    func updateItemStatus(itemId: String, isTaken: Bool, comment: String) async throws -> Item {
        let url = "\(baseUrl)/items/\(itemId)/status"
...
//    func getItemHistoryPage(itemId: String, page: Int = 1, size: Int = 20) async throws -> PagedList<History> {
//        let url = "\(baseUrl)/items/\(itemId)/history?page=\(page)&size=\(size)"
//        let historyPage: PagedList<History> = try await HttpClient.shared.getAsync(url)
//        return historyPage
//    }
